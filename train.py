...
from nmt_utils import chinese_tokenizer_load, english_tokenizer_load, malaysian_tokenizer_load, XLNET_chinese_tokenizer_load
...
def run_epoch(data, model, loss_compute):
    ...
    for batch in tqdm(data):
        out = model(batch.src, batch.trg, batch.src_mask, batch.trg_mask, batch.src_token_type_ids)
...
def evaluate(data, model, mode='dev', use_beam=True):
    ...
    if config.TRAN_ORI == 'en2ms':
        sp_chn = malaysian_tokenizer_load()
        if config.TRAN_REVERSE:
            sp_chn = english_tokenizer_load()
    if config.TRAN_ORI == 'zh2ms':
        sp_chn = malaysian_tokenizer_load()
        if config.TRAN_REVERSE:
            sp_chn = XLNET_chinese_tokenizer_load()
    trg = []  # TGT
    res = []  # 翻译结果
    with torch.no_grad():
        # 在data的英文数据长度上遍历下标
        for batch in tqdm(data):
            ...
            src_mask = batch.src_mask.unsqueeze(-2)
            if use_beam:
                decode_result, _ = beam_search(model, src, src_mask, config.max_len,
                                               config.padding_idx, config.bos_idx, config.eos_idx,
                                               config.beam_size, config.device, batch.src_token_type_ids)
            else:
    ...
    if mode != 'test':
        with open(config.output_path, "w", encoding="utf-8") as fp:
            for i in range(len(trg)):
                ...
                fp.write(line)
    trg = [trg]
    if config.TRAN_ORI == 'zh2en' or config.TRAN_ORI == 'zh2ms' and config.TRAN_REVERSE:
...
def test(data, model, criterion):
    with torch.no_grad():
        ...
        test_loss = run_epoch(data, model_par,
                              MultiGPULossCompute(model.generator, criterion, config.device_id, None))
        bleu_score = evaluate(data, model, 'test123')
